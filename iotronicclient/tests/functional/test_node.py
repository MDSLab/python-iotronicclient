...
from iotronicclient.tests.functional import base
import iotronicclient.tests.functional.utils as utils


class NodeSanityTestIotronicClient(base.FunctionalTestBase):
    """Sanity tests for testing actions with Node.

    Smoke test for the Iotronic CLI commands which checks basic actions with
    node command like create, delete etc.
    """

    def setUp(self):
        super(NodeSanityTestIotronicClient, self).setUp()
    ...
    def test_node_set_console_mode(self):
        ...
        self.iotronic('node-set-console-mode',
    ...
    def test_node_get_console(self):
        ...
        """
        node_show = self.show_node(self.node['uuid'])
        node_get = self.iotronic('node-get-console', params=self.node['uuid'])
