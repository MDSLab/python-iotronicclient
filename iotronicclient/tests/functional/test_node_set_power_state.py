...
class NodeSetPowerStateTestIotronicClient(base.FunctionalTestBase):
    """Tests for testing node-set-power-state command.

    Tests for the Iotronic CLI node-set-power-state command that checks that
    node can be set to 'on', 'off' or 'reboot' power states
   """

    def setUp(self):
        super(NodeSetPowerStateTestIotronicClient, self).setUp()
