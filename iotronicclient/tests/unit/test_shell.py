...
from iotronicclient import shell as iotronic_shell
from iotronicclient.tests.unit import utils
...
class ShellTest(utils.BaseTestCase):
    ...
    def shell(self, argstr):
        ...
        try:
            ...
            _shell = iotronic_shell.IotronicShell()
    ...
    def test_password_prompted(self, mock_getpass, mock_stdin, mock_client):
        ...
        expected_kwargs = {
            'iotronic_url': '', 'os_auth_url': FAKE_ENV['OS_AUTH_URL'],
            ...
            'os_iotronic_api_version': None, 'timeout': 600, 'insecure': False
        }
    ...
    def test_iotronic_api_version(self):
        self.shell('--iotronic-api-version 1.2 help')
        self.shell('--iotronic-api-version latest help')
        self.assertRaises(exc.CommandError,
                          self.shell, '--iotronic-api-version 1.2.1 help')
...
class TestCase(testtools.TestCase):
    ...
    def register_keystone_v2_token_fixture(self, request_mocker):
        ...
        service = v2_token.add_service('iot')
        service.add_endpoint('http://iotronic.example.com', region='RegionOne')
        request_mocker.post('%s/tokens' % V2_URL,
                            json=v2_token)

    def register_keystone_v3_token_fixture(self, request_mocker):
        v3_token = ks_fixture.V3Token()
        service = v3_token.add_service('iot')
        service.add_standard_endpoints(public='http://iotronic.example.com')
        request_mocker.post(
    ...
    def test_node_list(self, request_mocker):
        ...
        resp_dict = {"nodes": [
                     {"instance_uuid": "null",
                      ...
                      "links": [{"href": "http://iotronic.example.com:6385/"
                                 "v1/nodes/foo",
                                 "rel": "self"},
                                {"href": "http://iotronic.example.com:6385/"
                                 "nodes/foo",
                                 "rel": "bookmark"}],
                      "maintenance": "false",
        ...
        request_mocker.get('http://iotronic.example.com/v1/nodes',
                           headers=headers,
                           json=resp_dict)
...
class ShellParserTest(TestCase):
    def test_deprecated_defaults(self):
        ...
        parser = iotronic_shell.IotronicShell().get_base_parser()
